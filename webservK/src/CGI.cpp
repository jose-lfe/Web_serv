...
std::string exec_cgi(const handleRequest& req, const ServerConfig& configs, const Location *loc, std::string relPath)
{
    ...
    std::string scriptPath = get_pwd_path(configs) + "/" + loc->root + relPath;
...
if (req.method == "POST" && !req.body.empty()) {
    ...
    std::cout << "Total written to CGI stdin: " << total_written << " bytes" << std::endl;
}
    std::cout << "Body size: " << req.body.size() << " / Content-Length: " << getHeaderValue(req.headers, "Content-Length") << std::endl; // pour debug
    close(in_pipe[1]);
    ...
    if (wpid == -1)
    {
        ...
        if (WIFEXITED(status)) {
            ...
            if (exit_code != 0)
            {
                // Le CGI a renvoyé une erreur
                return buildErrorResponse(500, configs.error_pages);
            }
        } else if (WIFSIGNALED(status)) {
            // L’enfant a été tué par un signal
    ...
    std::cout << "newbody size: " << newbody.size() << " body size: " << body.size() << std::endl;
