...
HttpServer& HttpServer::operator=(const HttpServer &other)
{
    if (this != &other)
    {
        ...
        _parsedRequests = other._parsedRequests;
...
void HttpServer::run()
{
    ...
    close(epoll_fd);
    throw std::runtime_error("Signal detected. Ending program.");
}
...
void HttpServer::handleRead(int fd, int epoll_fd, Client &client) {
    ...
    epoll_event ev;
    ev.events = client._responseQueue.empty() ? EPOLLIN : (EPOLLIN | EPOLLOUT);
    ev.data.fd = fd;
    if (epoll_ctl(epoll_fd, EPOLL_CTL_MOD, fd, &ev) < 0) {
        std::cerr << "Error: epoll_ctl: mod to EPOLLOUT" << std::endl;
...
void HttpServer::acceptClient(int listen_fd, int epoll_fd, int port) {
    ...
    std::cerr << "Error: fcntl client_fd O_NONBLOCK" << std::endl;
    close(client_fd);
    return;
}
...
void HttpServer::handleWrite(int fd, int epoll_fd, Client &client) {
	...
	if (epoll_ctl(epoll_fd, EPOLL_CTL_MOD, fd, &ev) < 0)
	{
		std::cerr << ("Error: epoll_ctl: mod to EPOLLIN") << std::endl;
		closeClient(fd, epoll_fd);
		return;
	}
