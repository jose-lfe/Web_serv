            ...
            if (tokens.size() >= 2) {
                ...
                if (key == "listen") {
                    ...
                    if (in_location)
                        ...
                        current.autoindex = stringToBool(value);
                } else if (key == "allow_methods") {
                    if (in_location) {
                        current_location.methods.clear();
                        for (size_t i = 1; i < tokens.size(); ++i)
                            current_location.methods.push_back(removeSemicolon(tokens[i]));
                    }
				...
				{
					throw std::runtime_error("Error: invalid token in configuration file");
				}
            }
        }
		else
		{
			throw std::runtime_error("Error: invalid header in configuration file");
		}
    }
    return servers;
}
