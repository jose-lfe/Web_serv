...
std::string buildRedirectionResponse(const std::string& location, const std::string& userAgent = "", const std::string& status = "303 See Other") {
    ...
    // Détection basique de curl
    bool isCurl = userAgent.find("curl") != std::string::npos;

    if (isCurl) {
        std::string body = "Resource deleted. Redirecting to: " + location + "\n";
        ...
        body << "<html><head><meta http-equiv=\"refresh\" content=\"1; URL=" << location << "\">"
...
std::string SimpleRouter::route(const HandleRequest& req, const std::vector<ServerConfig>& _configs) {
    ...
    if ((method == "POST" && path.find("/photos/") == 0 && extractQueryString(path).find("_method=DELETE") != std::string::npos) || method == "DELETE")
    {
		handleDelete(req, _configs);
