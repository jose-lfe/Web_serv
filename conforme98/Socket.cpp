...
// Constructeur de copie (fd non copié exprès)
Socket::Socket(const Socket& other)
    : _fd(-1), _host(other._host), _port(other._port), _addr(other._addr) {
    std::cerr << "[Socket] Copie d'un socket (fd non copié)\n";
    _servers = other._servers;
}

// Opérateur d'affectation (fd non copié exprès)
Socket& Socket::operator=(const Socket& other) {
    if (this != &other) {
        ...
        _addr = other._addr;
        _servers = other._servers;
        std::cerr << "[Socket] Affectation d'un socket (fd non copié)\n";
    }
    return *this;
}
